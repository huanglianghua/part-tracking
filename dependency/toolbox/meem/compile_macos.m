...
function [cflags,libs] = pkg_config(opts)
    ...
    l_options = { '-lopencv_core -lopencv_highgui -lopencv_imgproc' };
    if opts.debug
...
function l = lib_names(L_path)
    ...
    d = dir( fullfile(L_path,'*opencv_*.dylib') );
    l = regexp({d.name}, '(opencv_core|(opencv_imgproc.+)\.dylib|(libopencv_highgui.+)\.dylib', 'tokens', 'once');
    l = [l{:}];
    l = 'opencv_core -lopencv_highgui -lopencv_imgproc';
end